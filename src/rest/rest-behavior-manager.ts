...
export class RestBehaviorManager {
    ...
    public addEntity(entity: Computed): this {
    ...
    public attachRules(cl: Computed, methods: RequestMethod[], handlers: PrioritizedHandler[]): this {
        try {
    ...
    public addMiddleware(cl: Computed, method: RequestMethod, handler: PrioritizedHandler): this {
    ...
    public getEntityPrototype(path: string): Computed | null {
        if (!this.entityPrototypesMap[path]) {
            return null;
        }
        const p = this.entityPrototypesMap[path];
        if (p ! instanceof Computed) {
            return null;
        }
